...
% Each script manages its own StageClient connection internally.
    ...
    autoSave1 = uicontrol(tab1, 'Style', 'checkbox', ...
        'String', 'Auto-save brightness CSV', ...
        'Value', 1, ...
        'Position', [20 300 220 22], ...
    ...
    autoSave2 = uicontrol(tab2, 'Style', 'checkbox', ...
        'String', 'Auto-save brightness CSV', ...
        'Value', 1, ...
        'Position', [20 195 220 22], ...
        'ForegroundColor', [0.85 0.85 0.9], ...
        'BackgroundColor', [0.18 0.18 0.22], ...
        'FontSize', 10);
    ...
    function runFlicker()
        try
            ...
            % Auto-save brightness CSV
            if get(autoSave1, 'Value')
                setStatus('Auto-saving flicker brightness CSV...');
                drawnow;
                saveFlickerForAnalysis(hz, sf, rr, colorMode);
            end
    ...
    function runNoise()
        try
            ...
            % Auto-save brightness CSV
            if get(autoSave2, 'Value')
                setStatus('Auto-saving noise brightness CSV...');
                drawnow;
                saveFullFieldNoiseForAnalysis(sd, m, sg, hz, sf, rr, colorMode);
            end
    ...
    function runCheckerboard()
        try
            ...
            if get(autoSave3, 'Value')
                setStatus('Auto-saving noise sequence (.mat + .csv)...');
                drawnow;
                saveCheckerboardNoiseForAnalysis(sd, m, sg, 40, 32, sf, un, colorMode);
            end
    ...
    function saveNoiseSeq()
        setStatus('Saving noise sequence (.mat + .csv)...');
        drawnow;
        try
            sd = str2double(get(seed3, 'String'));
            m  = str2double(get(mu3, 'String'));
            ...
            saveCheckerboardNoiseForAnalysis(sd, m, sg, 40, 32, sf, un, colorMode);
            setStatus(sprintf('Noise for seed=%d saved (.mat + .csv).', sd));
        catch err
            setStatus(['ERROR: ' err.message]);
    ...
    % ABORTS if Clampex window cannot be found (prevents silent failures).
        preDelay  = str2double(get(preDelayEdit, 'String'));
        postDelay = str2double(get(postDelayEdit, 'String'));

        % 1. Bring Clampex to foreground (abort if not found)
        setStatus('Activating Clampex window for recording START...');
        drawnow;
        if ~focusClampexSafe()
            setStatus('ERROR: Clampex window not found. Aborting trial.');
            error('MCP:ClampexNotFound', 'Cannot find Clampex window. Is it open?');
        end
        pause(0.3);

        % 2. Start gap-free recording (Ctrl+Shift+1)
        fprintf('[MCP] Clampex recording START at %s\n', datestr(now, 'HH:MM:SS.FFF'));
        ...
        fprintf('[MCP] Pre-stimulus delay (%.1f s) at %s\n', preDelay, datestr(now, 'HH:MM:SS.FFF'));
        setStatus(sprintf('Pre-stim delay (%.1f s)...', preDelay));
        drawnow;
        pause(preDelay);

        % 5. Run the stimulus
        fprintf('[MCP] Stimulus START at %s\n', datestr(now, 'HH:MM:SS.FFF'));
        setStatus('Running stimulus...');
        drawnow;
        stimFn();
        fprintf('[MCP] Stimulus DONE at %s\n', datestr(now, 'HH:MM:SS.FFF'));

        % 6. Post-stimulus delay
        fprintf('[MCP] Post-stimulus delay (%.1f s) at %s\n', postDelay, datestr(now, 'HH:MM:SS.FFF'));
        ...
        drawnow;
        if ~focusClampexSafe()
            setStatus('WARNING: Could not refocus Clampex to STOP recording!');
            fprintf(2, '[MCP] WARNING: Failed to focus Clampex for STOP at %s\n', datestr(now, 'HH:MM:SS.FFF'));
        end
        pause(0.3);

        % 8. Stop recording (same toggle: Ctrl+Shift+1)
        fprintf('[MCP] Clampex recording STOP at %s\n', datestr(now, 'HH:MM:SS.FFF'));
        System.Windows.Forms.SendKeys.SendWait('^+{1}');
        pause(0.1);

        % 9. Return focus to MATLAB
        focusMatlab();
        pause(0.5);  % Longer pause to let Clampex finalize .abf file
    ...
    % If recording is enabled, each trial is wrapped with Clampex.
        ...
        fprintf('\n========================================\n');
        fprintf('[MCP] EXPERIMENT START: %s\n', stimName);
        fprintf('[MCP] Time: %s\n', datestr(now, 'yyyy-mm-dd HH:MM:SS.FFF'));
        fprintf('[MCP] Trials: %d, Recording: %s\n', nTrials, mat2str(useRec));
        fprintf('[MCP] Pre: %s s, Post: %s s, ITI: %s s\n', ...
            get(preDelayEdit, 'String'), get(postDelayEdit, 'String'), get(itiEdit, 'String'));
        fprintf('========================================\n\n');

        for trial = 1:nTrials
            setStatus(sprintf('Trial %d/%d: %s...', trial, nTrials, stimName));
            fprintf('[MCP] === Trial %d/%d: %s === at %s\n', trial, nTrials, stimName, datestr(now, 'HH:MM:SS.FFF'));
            drawnow;

            if useRec
                runWithRecording(stimFn);
            else
                fprintf('[MCP] Stimulus START (no recording) at %s\n', datestr(now, 'HH:MM:SS.FFF'));
                stimFn();
                fprintf('[MCP] Stimulus DONE at %s\n', datestr(now, 'HH:MM:SS.FFF'));
            end

            fprintf('[MCP] Trial %d/%d COMPLETE at %s\n', trial, nTrials, datestr(now, 'HH:MM:SS.FFF'));
        ...
        fprintf('\n========================================\n');
        fprintf('[MCP] EXPERIMENT COMPLETE: %s at %s\n', stimName, datestr(now, 'HH:MM:SS.FFF'));
        fprintf('[MCP] Total trials: %d\n', nTrials);
        fprintf('========================================\n\n');
    end

    % ---- Focus Helpers (robust window targeting) ----
    function success = focusClampexSafe()
    % Activates the Clampex window by searching for its title.
    % Returns TRUE if Clampex was found and activated, FALSE otherwise.
    % Uses onCleanup for guaranteed COM object cleanup (prevents leaks).
        success = false;
        try
            shell = actxserver('WScript.Shell');
            cleanupObj = onCleanup(@() delete(shell));
            success = shell.AppActivate('Clampex');
            if ~success
                success = shell.AppActivate('pCLAMP');
            end
            if ~success
                fprintf(2, '[MCP] WARNING: Could not find Clampex window at %s\n', datestr(now, 'HH:MM:SS.FFF'));
            end
        catch err
            fprintf(2, '[MCP] ERROR in focusClampexSafe: %s\n', err.message);
            success = false;
        end
    end
    ...
    % Uses onCleanup for guaranteed COM object cleanup (prevents leaks).
        try
            ...
            % Silently fail - we are already in MATLAB
        end
    end
end
